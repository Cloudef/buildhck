...
def clone(srcdir, url, branch, result):
    ...
    if os.path.isdir(os.path.join(srcdir, '.git')):
        ...
        if not _git('fetch', '-f', '-u', 'origin', '{}:{}'.format(branch, branch)):
            raise DownloadException('git fetch failed')
        if not _git('checkout', '-f', branch):
    ...
    os.chdir(srcdir)
    result['commit'] = _git2('rev-parse', 'HEAD').strip().decode('UTF-8')
    result['description'] = _git2('log', '-1', '--pretty=%B').strip().decode('UTF-8')
